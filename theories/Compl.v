...
Module Type COMPL_F(Import Conf : DETECT_CONF)(Import Params : DETECT_PARAMS(Conf)).
  ...
  Notation "'' MN" := (deinstr MN) (at level 1).
  ...
  Lemma flushed_in_NoRecvQ_MQ : forall MN n, flushed_in n MN -> NoRecvQ_MQ (mserv_q (MN n)).
  Proof. unfold flushed_in, NetGet. intros. destruct (NetMod.get n MN); attac. Qed.
  Lemma flushed_in_NoRecvR_MQ : forall MN n, flushed_in n MN -> NoRecvR_MQ (mserv_q (MN n)).
  Proof. unfold flushed_in, NetGet. intros. destruct (NetMod.get n MN); attac. Qed.

  #[export] Hint Immediate flushed_in_NoRecvQ_MQ flushed_in_NoRecvR_MQ : LTS.

  Lemma flushed_NoRecvQ_from [MN n] v : flushed_in n MN -> NoRecvQ_MQ (mserv_q (MN n)) -> NoRecvQ_from v (mserv_q (MN n)).
  Proof.
    unfold NoRecvQ_from in *. intros. intros ?.
    ltac1:(autounfold with LTS_get in * ). destruct (NetMod.get n MN).
    induction mserv_q0; kill H0; attac.
  Qed.
  ...
  Lemma net_vis_MqRecvQ_pop [MN0 MN1 a n m n'] :
    (MN0 ~(n' @ a)~> MN1) ->
    ~ NoRecvQ_from n (mserv_q (MN0 m)) ->
    NoRecvQ_from n (mserv_q (MN1 m)) ->
  ...
  Proof.
    ...
    - consider (exists v, a0 = MActP (Recv (n, Q) v) /\ n0 = m) by (eapply net_vis_MqRecvQ_pop; eauto).
      exists v. auto.
    - assert (NoRecvQ_from n (mserv_q (N0' m)) \/ ~ NoRecvQ_from n (mserv_q (N0' m))) as [|] by eauto using NoRecvQ_from_dec.
  ...
  Proof.
    ...
    assert ('' N0 =(@NComm _ gen_Act_PAct n0 n1 Q v)=> '' N1)
      by (eapply deinstr_act_do; eauto with LTS; simpl; eauto with LTS).
  ...
  Proof.
    ...
    intros.
    destruct x; auto.
    destruct (serv_deinstr (mserv MQ0 M0 (serv I0 P0 O0))) as [I' P' O'] eqn:?.
  ...
  Proof.
    ...
    - assert ('' N0 =(p)=> '' N1) by eauto using deinstr_act_do.
      ...
      + destruct p; doubt.
        attac.
    - assert ('' N0 = '' N1) by eauto using deinstr_act_skip.
      rewrite `('' N0 = _) in *.
      bs.
  Qed.
  ...
  Proof.
    ...
    hsimpl in *.
    eapply (dead_set_lock_set `(dead_set DS '' MN0)) in H0.
    hsimpl in *.
    unfold lock_set in *.
  ...
  Proof.
    ...
    split.
    - destruct (NetMod.get n (deinstr MN0)) as [I0 P0 O0] eqn:?.
      ...
      enough (P = P0 /\ List.In (n0, R, v) I0) as [? ?].
      {
        ...
        replace (serv_deinstr (NetMod.get n MN0)) with (NetMod.get n (deinstr MN0)) by (unfold deinstr, serv_deinstr; attac).
    ...
    - assert (lock_set MN0 [n'] n) by eauto using lock_singleton with LTS.
      ...
      enough (NetMod.get n (deinstr MN0) = serv (&I ++ retract_recv MQ) P (retract_send MQ ++ &O))
  ...
  Proof.
    ...
    consider (exists L, lock_set '' MN0 L n /\ incl L x)
      by eauto using dead_set_lock_set.
  ...
  Proof.
    ...
    - assert (locked (MN0 n0) = Some n1) by auto.
      ...
      + consider (exists v, na = NTau n0 (MActP (Recv (n1, R) v))) by eauto using Alg_set_set_unlocked with LTS.
        exfalso.
        assert (deinstr MN0 = MN1) by (eapply deinstr_act_skip; eauto; simpl; eauto).
        rewrite `(deinstr MN0 = _) in *.
  ...
  Proof.
    ...
    assert (exists path, deinstr MN0 =[ path ]=> MN1) as [? ?] by eauto using transp_sound1.
  ...
  Proof.
    intros.
    consider (exists ppath, deinstr MN0 =[ppath]=> MN1) by eauto using transp_sound1.
    eauto with LTS.
  Qed.
  ...
  Proof.
    ...
    - consider (exists ppath, deinstr MN0 =[ppath]=> N1) by eauto using transp_sound1.
  ...
  Lemma deinstr_act_or [MN0 MN1 : MNet] [ma] :
    (MN0 =(ma)=> MN1) ->
    deinstr MN0 = MN1 \/ exists a, deinstr MN0 =(a)=> MN1.

  Proof.
    clear.
    intros.
    destruct (MNAct_to_PNAct ma) eqn:?.
    - right. exists p. eauto using deinstr_act_do.
    - left. eauto using deinstr_act_skip.
  Qed.
  ...
  Proof.
    intros.
    destruct (@deinstr_act_or MN0 MN1 a); auto.
    - rewrite <- `(deinstr MN0 = _). auto.
  ...
  Lemma KIC_invariant_H_wait [MN0 MN1 : MNet] [a] [n0 n1] :
    ...
    (MN0 =(a)=> MN1) ->
    lock MN1 n0 n1 ->
    NoRecvQ_from n0 (mserv_q (MN1 n1)) ->
    List.In n0 (wait (MN1 n1)).

  Proof.
    intros.
    assert (lock MN0 n0 n1 \/ ~ lock MN0 n0 n1) as [|] by eauto using well_formed_lock_dec with LTS.
    - assert (NoRecvQ_from n0 (mserv_q (MN0 n1)) \/ ~ NoRecvQ_from n0 (mserv_q (MN0 n1))) as [|] by eauto using NoRecvQ_from_dec.
    ...
    - consider (exists v, a = NComm n0 n1 Q (MValP v)) by eauto using SRPC_M_net_new_lock_query with LTS.
      absurd (List.In (MqRecv (n0, Q) v) (mserv_q (MN1 n1))). 1: attac.
  ...
  Proof with eattac.
    ...
    destruct (dead_set_dep_set HDS HIn) as [L HL].

    assert (L <> []) as HLnil.
    {
      destruct L; doubt.
      destruct (dead_set_lock_set HDS HIn) as [L' [HL' _]].
    ...
    specialize (dead_set_dep_set_dead_set HDS HL HLnil HIn) as HDSL.
    ...
    {
      apply (dead_set_lock_set HDSL) in HIn2 as [L2 [HL2 _]].
  ...
  Lemma sends_probe_prop_foreign [MN0 n0 n1 n2 p MQ] :
    ...
    lock MN0 n1 n2 ->
    origin p <> n1 ->
    (* List.In (MqProbe (n2, R) p) (mserv_q (MN0 n1)) -> *) (* TODO requires tighter KIC-wait *)
    mserv_q (MN0 n1) = MQ ++ [MqProbe (n2, R) p] ->
  ...
  Proof.
    ...
    (* assert (mserv_q (MN0 n1) = MQ1) by (ltac1:(autounfold with LTS_get in * ); attac). *)
    (* rewrite `(mserv_q (MN0 n1) = _) in *.  clear H4. *)
    consider (MQ1 = mserv_q (MN0 n1)) by (ltac1:(autounfold with LTS_get in * ); attac).
    rewrite `(mserv_q (MN0 n1) = _) in *. clear H3.
    ...
    - enough (NoRecvR_MQ MQ) by eauto with LTS.
      enough (NoRecvR_MQ (mserv_q (MN0 n1))) by (ltac1:(autounfold with LTS_get in * ); attac).
    ...
    - consider (serv_client n0 (NetMod.get n1 '' MN0)) by eauto with LTS.
      ...
      + enough (NoRecvQ_from n0 (MQ ++ [MqProbe (n2, R) p])).
        ...
        assert (NetMod.get n1 '' MN0 = serv &I P &O).
        {
          ...
          unfold deinstr, serv_deinstr; hsimpl in *; hsimpl in *; attac.
        }
        eauto with LTS.
  ...
  Proof.
    ...
    assert (NoRecvR_MQ (mserv_q (MN1 n1))) by eauto using locked_M_NoRecvR with LTS.

    assert (NoSends_MQ (mserv_q (MN1 n1))).
    {
    ...
    assert (NoRecvQ_from n0 (mserv_q (MN1 n1)) -> List.In n0 (wait (MN1 n1)))
  ...
  Proof.
    ...
    consider (exists MQdeinstr Mdeinstr,
                 NetMod.get m MN2 = mserv ((MQ ++ MQ') ++ MQdeinstr) Mdeinstr &S
                 /\ (n <> m -> Mdeinstr = M')
                 /\ MQ_Clear MQdeinstr).

    exists (MQ' ++ MQdeinstr), Mdeinstr.
    smash_eq m n; hsimpl.
  ...
  Lemma flush_one_In [MN0 : MNet] [n e] :
    List.In e (mserv_q (MN0 n)) ->
    exists MQ00 MQ01 MQ1' M1 S1 mpath MN1,
      (mserv_q (MN0 n)) = MQ00 ++ e :: MQ01
      /\ (MN0 =[mpath]=> MN1)
      /\ Forall (Flushing_NAct n) mpath
  ...
  Lemma propagation_init [MN0 : MNet] [n n' m] [v ] :
    ...
    deadlocked n '' MN0 ->
    lock MN0 n n' ->
    List.In (MqRecv (m, Q) v) (mserv_q (MN0 n)) ->
    exists MN1 mpath MQ,
      (MN0 =[mpath]=> MN1)
      /\ detect_path [n] mpath
      /\ mserv_q (MN1 m) = MQ ++ [active_ev_of MN1 n n].
  ...
  Proof.
    ...
    assert (exists MN3 MQ, (MN2 =(NComm n m R (MValM (active_probe_of MN2 n)))=> MN3) /\ mserv_q (MN3 m) = MQ ++ [active_ev_of MN3 n n])
      as Hxx.
    {
  ...
  Lemma sends_probe_send [MN0 : MNet] [n m p] :
    ...
    exists MN1 mpath MQn',
      (MN0 =[mpath]=> MN1)
      /\ detect_path [m] mpath
      /\ ((alarm (MN1 m) = true /\ m = origin p) \/ (mserv_q (MN1 n) = MQn' ++ [MqProbe (m, R) p])).
    ...
    - consider (sends_probe _ _ _).
      + consider (exists m', lock MN0 m m') by eauto using deadlocked_M_get_lock with LTS.
        consider (exists (MN1 : MNet) mpath MQ,
                     (MN0 =[mpath]=> MN1)
                     /\ detect_path [m] mpath
                     /\ mserv_q (MN1 n) = MQ ++ [active_ev_of MN1 m m])
      ...
      + assert (mserv_q (MN0 m) = MQ0 ++ MqProbe (n', R) p :: MQ') as Hget0
        ...
        assert (List.In n (wait (MN1 m))).
        {
          ...
          - hsimpl in H8.
            ...
            assert (lock MN1 n m) by
              (consider (exists ppath, '' MN0 =[ppath]=> '' MN1) by eauto using transp_sound with LTS;
               eauto 10 with LTS).
            enough (NoRecvQ_from n (mserv_q (MN1 m))) by (consider (KIC MN1)).
        ...
        * enough (exists MN3 mpath1 MQn',
                     ...
                     /\ mserv_q (MN3 n) = MQn' ++ [MqProbe (m, R) {| origin := origin0; lock_id := lock_id0 |}]).
          {
            hsimpl in *.
          ...
          assert (exists MQ3 MN2 MQn',
                     ...
                     /\ (mserv_q ((NetMod.put m (mserv MQ3 (MSend_all wait0 R p M ) S1) MN2) a)) = MQn' ++ [MqProbe (m, R) p]
                 ).
          {
            smash_eq m a.
      ...
      + have (well_formed '' MN1) by eauto with LTS.
        ...
        consider (exists (MN2 : MNet) (mpath : list MNAct) MQn',
                     ...
                     /\ ((alarm (MN2 m) = true /\ m = origin p)
                         \/ mserv_q (MN2 n) = MQn' ++ [MqProbe (m, R) p])
                 ) by
          re_have (
              consider (exists ppath, '' MN0 =[ppath]=> '' MN1) by eauto using transp_sound1;
  ...
  Lemma detection_finito [MN0 : MNet] [n m] :
    ...
    deadlocked n '' MN0 ->
    lock MN0 n m ->
    List.In (active_ev_of MN0 m n) (mserv_q (MN0 n)) ->
    exists MN1 mpath,
      (MN0 =[mpath]=> MN1)
      /\ detect_path [n] mpath
      /\ alarm (MN1 n) = true.
  ...
  Lemma in_sends_probe [MN0 n' n m p MQ] :
    ...
    deadlocked m '' MN0 ->
    active MN0 p (origin p) ->
    mserv_q (MN0 n) = MQ ++ [MqProbe (m, R) p] ->
  ...
  Proof.
    ...
    destruct `((alarm (MN1 m) = true /\ _) \/ mserv_q (MN1 n) = MQn' ++ [MqProbe (m, R) p]) as [|].
    1: eattac.
  ...
  Proof.
    ...
    ltac1:(rev_induction L); intros; hsimpl in *.
    {
      ...
      - assert (lock MN1 n m).
        ...
        repeat split; auto.
        eapply detect_path_incl > [| eattac]; ieattac.
        destruct `(_ \/ _); subst; eauto using dead_set_in.
    }
    ...
    1: {
      ...
      ieattac.
      destruct `(_ \/ _); subst; eauto using dead_set_in.
      eauto using dead_set_dep_in with LTS.
    }
    ...
    assert (In m DS).
    {
      eapply dead_set_dep_in. eauto. eapply H6. subst. eauto 3 with LTS.
    }
    ...
    specialize (H ltac:(exists DS; eauto 3 using dead_set_dep_in with LTS)).
    hsimpl in H.

    exists MN2, (mpath0 ++ mpath), n'; repeat split > [eattac | | eattac].
    assert (In a DS) by eauto 3 using dead_set_dep_in with LTS.
    ...
    destruct `(_ \/ _); subst.
    assert (In a0 DS) by eauto 3 using dead_set_dep_in with LTS.
    attac.
    bs.
  Qed.
  ...
  Proof.
    ...
    have (deadlocked m '' MN0) by (exists DS; eauto using dead_set_dep_in with LTS).
    have (deadlocked m' '' MN0) by (exists DS; eauto using dead_set_in with LTS).
    assert (active MN1 p (origin p)).
    {
      consider (active MN0 p n).
      constructor; auto.
    ...
    consider (exists MN2 mpath1 MQn',
                 (MN1 =[mpath1]=> MN2)
                 /\ detect_path [n'] mpath1
                 /\ ((alarm (MN2 n') = true /\ n' = origin p) \/ (mserv_q (MN2 n) = MQn' ++ [MqProbe (n', R) p]))).
    {
      eapply sends_probe_send; re_have eauto with LTS.
    }

    destruct `((alarm (MN2 n') = true /\ n' = origin p) \/ mserv_q (MN2 n) = MQn' ++ [MqProbe (n', R) p]).
    1: { exists (mpath0 ++ mpath1), MN2; repeat split > [eattac | | eattac].
         assert (In n' DS). assert (dead_set DS '' MN1) by eauto 3 with LTS. eauto 4 using dead_set_in with LTS.
    ...
    assert  (List.In (active_ev_of MN2 n' n) (mserv_q (MN2 n))).
    {
      unfold active_ev_of.
      ...
      rewrite <- `(p = _).
      rewrite `(mserv_q (_ _) = _).
      auto with datatypes.
    }
    have (List.In (active_ev_of MN2 n' n) (mserv_q (MN2 n))) by (unfold active_ev_of; eauto with LTS).
  ...
  Lemma mserv_Q_lock_sound [MN m n v] :
    well_formed '' MN ->
    List.In (MqRecv (m, Q) v) (mserv_q (MN n)) ->
    lock MN m n.
  Proof.
    intros.
    ...
    hsimpl in |- *.
    destruct ((serv_deinstr (NetMod.get n MN))) eqn:?.
    econstructor 1; attac.
    eapply serv_deinstr_In_recv. eauto.
    rewrite Heqm0 in *.
    eauto.
  Qed.
  ...
  Proof.
    ...
    induction mpath; attac.
    rename N1 into MN0'.
    transitivity '(deinstr MN0').
    - eapply locked_serv_deinstr; eauto with LTS.
      exists DS; eattac.
    - apply deinstr_act in H2.
  ...
  Proof.
    ...
    + smash_eq n0 n.
        * consider (_ =(_)=> _); hsimpl in *.
          destruct m; attac.
          -- destruct p; doubt.
             unfold deinstr, serv_deinstr, ready_in, ready, apply_instr, instr_for, NetGet in *.
             compat_hsimpl in *; attac.
          -- destruct a; doubt.
             unfold deinstr, serv_deinstr, ready_in, ready, apply_instr, instr_for, NetGet in *.
             compat_hsimpl in *; attac.
  ...
  Proof.
    ...
    replace (deinstr MN0) with (deinstr MN1)
      by (symmetry; eapply locked_serv_deinstrs; eauto with LTS).

    exists (mpath0 ++ mpath1), i1.

    assert (KIC MN1) by eauto with LTS.
    eauto using net_preserve_alarm_path, dead_set_dep_in with LTS.
  Qed.
