...
Module Type LOCKS(Name : UsualDecidableSet)(NetModF : NET).
  ...
  Module Import NetLocks.
    ...
    Fact net_lock_move_eq L n N0 N1 :
      ...
      net_lock N0 L n0 <-> net_lock N1 L n0.
...
Module Srpc(Name : UsualDecidableSet)(NetModF : NET).
  ...
  (** SRPC process handling a request from the client [c] *)
    ...
    Lemma SRPC_recv_no_lock_r [S0 S1 nc v L] :
      AnySRPC_pq S0 ->
      (S0 =(recv nc v)=> S1) ->
      ~ pq_lock L S1.
    ...
    Proof.
      ...
      assert (net_lock_on N0 m0 m1).
      { (* TODO derive lock bad sender, tag etc *)
        fail.
        unfold net_lock_on, net_lock in *; hsimpl in *.
        exists L0. split > [attac|].
    ...
    Proof.
      ...
      { (* TODO derive lock bad sender, tag etc *)
        ...
        exists L0. split > [attac|].
        hsimpl in *.
        consider (exists srpc, SRPC_sane srpc (NetMod.get m1 N0)) by attac.
        bullshit (~ pq_lock L0 &S) by eauto using SRPC_sane_send_R_no_lock_r.
      }
